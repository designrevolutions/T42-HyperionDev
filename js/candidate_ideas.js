...
let comments_array = []; // This is for storing the comments left by users
...
// ########### ~~~~~~~~~
class Comments_Data // I'm adding this for making comments and storing for each page
{
...
function on_page_load() // 1. This is the first function that gets called.
{
    ...
    load_comments(); // We load comments if any left by users
}
...
function store_comments_array_to_local_storage()
{
    // Store comments to localStorage
    localStorage.setItem("comments", JSON.stringify(comments_array));
}

function retrieve_comments_array_from_local_storage()
{
    let check_comments_local_storage = JSON.parse(localStorage.getItem("comments")) 

    if(check_comments_local_storage) // This means localStorage has some data
    {
        comments_array = check_comments_local_storage
    }
    else
    {
        // Do nothing. Leave like_data_retrieved_array as it is.
...
function store_user_comment()
{
    let user_comment = document.getElementById("user_comment");
    // I tried using let user_comment = document.getElementById("user_comment").value;
    // But below when I set user_comment = "" to clear, it didn't work for some reason.
    // So, I had to use .value and set that to "".

    if(user_comment.value === "")
    {
        alert("You must leave some text to post a comment")
    }
    else
    {
        comments_array.push(new Comments_Data(this_page_url, user_comment.value));

        user_comment.value = "";

        store_comments_array_to_local_storage();

        load_comments()
    }

    // We need to save user comment to localStorage
    // We need to clear the textarea
    // We need to load the bottom area of the page and load comments for THIS page only
}

let comments_left = document.getElementById("comments_left"); // This is an existing div that we have in our HTML. We use this in our function below.

function load_comments()
{
    // Here we read from localStorage
    retrieve_comments_array_from_local_storage() // We'll have an empty array if nothing is retrieved

    if(comments_array) // If true, then we have data in the array that we can check
    {
        // We need to get an array where the first object is this page's URL
        // This code makes a new array, but INCLUDES the current page URL - we want to fill out our comments
        // We needed code to search the array in order to find matches
        // I got the solution from Chat GPT
        comments_array = comments_array.filter(data => data.url === this_page_url); // When we have used this filter function above, we used !==, this time we use ===

        // Now cycle through the array and create DIV components and leave the text there
        if(comments_array.length > 0) // If true, then we definitely have comments. I had to use the length here - when I used just the array, even though it was empty, it still returned true. This didn't happen above!
        {
            delete_all_comments_html();
            // console.log(comments_array)

            let extra_text = "Comments left by other users:"
            let paragraph = document.createElement("p");
            paragraph.innerText = extra_text;
            comments_left.appendChild(paragraph)

            comments_array.forEach(element => 
            {
                let comment_div = document.createElement("div");
                comment_div.innerText = element.comment;
                comment_div.className = "comments_left_div"

                comments_left.appendChild(comment_div)
            });
        }
    }
    else
    {
        // There aren't any comments. No action needed.
    }
}

function delete_all_comments_html() // We need to delete all comments before reading from array and filling in
{
    while (comments_left.firstChild)
    {
        comments_left.removeChild(comments_left.firstChild);
    }
}

let comment_submit_button = document.getElementById("comment_submit_button");
comment_submit_button.addEventListener("click", store_user_comment)
// ###### ~~~~~~~ #######
